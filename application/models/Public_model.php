...
class Public_model extends CI_Model
{
    ...
    private function getFilter($big_get)
    {
        ...
        if ($big_get['search_in_title'] != '') {
            ...
            $this->db->or_like('products_translations.description',$big_get['search_in_title']); 
            $this->db->group_end();  
    ...
    public function changePaytmOrderStatus($post, $status)
    { 
        $this->db->where('order_id', $post['ORDERID']);
        if (!$this->db->update('orders', array(
                    'status' => $status,
                    'paytm_status' => $post['STATUS'],
                    'txn_id' => $post['TXNID'],
                    'paid_amt' => $post['TXNAMOUNT'],
                    'created' => $post['TXNDATE'],
                    'check_sum_hash' => $post['CHECKSUMHASH']
                ))) {
            log_message('error', print_r($this->db->error(), true));
        }
    }

    public function changePaytmOrderStatusiffail($post, $status)
    { 
        $processed = 0;
        if ($status == 'canceled') {
            $processed = 1;
    ...
    public function getCodcity($city)
{       $this->db->where('city', $city);
    ...
    public function getSubcspecific($post){
        $this->db->where('id', $post['id']);
        $result = $this->db->get('subscription_details');
        return $result->result_array();
     }

     public function getSubcspecificfororders($id){
        $this->db->where('id', $id);
        $result = $this->db->get('subscription_details');
        return $result->result_array();
     }
     ...
     public function setSubOrder($post)
    {
        if (!$this->db->insert('subscription_orders', array(
        'user_id' => $post['user_id'],
     ...
     public function getMonthdetailsofUserforUserside($id ){
     ...
     public function GetallorderesCountforuser($id)
    {    $this->db->where('user_id', $id);
        $this->db->where('MONTH(date)', date("m"));
        $this->db->where('YEAR(date)', date("Y"));
