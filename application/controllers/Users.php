...
class Users extends MY_Controller
{
    ...
    public function mysubscription($page = 0)
    {
        if (isset($_POST['update'])) {
            $_POST['id'] = $_SESSION['logged_user'];
            $count_emails = $this->Public_model->countPublicUsersWithEmail($_POST['email'], $_POST['id']);
        ...
        $head = array();
        $data = array();
        $head['title'] = 'subscription-month history';
        $head['description'] = lang('my_acc');
        $head['keywords'] = str_replace(" ", ",", $head['title']);
        $data['userInfo'] = $this->Public_model->getUserProfileInfo($_SESSION['logged_user']);
        $rowscount = $this->Public_model->GetallorderesCountforuser($_SESSION['logged_user']);
        $data['orders'] = $this->Public_model->getMonthdetailsofUserforUserside($_SESSION['logged_user']);
        $data['links_pagination'] = pagination('mysubscription', $rowscount, $this->num_rows, 2);
        $this->render('subscriptionHistory', $head, $data);
    }
    ...
    public function getUserhistorybymonth(){
        ...
        $data['userInfo'] = $this->Public_model->getUserProfileInfo($_SESSION['logged_user']);
        $rowscount = $this->Public_model->GetallorderesCountforuser($_SESSION['logged_user']);
        $data['orders'] = $this->Public_model->getMonthdetailsofUser($_GET['user_id'],$_GET['month_date']);


        $rowscount = $this->Public_model->GetallorderesCountforuser($_SESSION['logged_user']);       

        $data['links_pagination'] = pagination('mysubscription/month', $rowscount, $this->num_rows, 3);    
        $this->render('subscriptionHistory', $head, $data);
    }
