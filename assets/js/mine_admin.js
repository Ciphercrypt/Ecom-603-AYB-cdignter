...
function changeOrdersOrderStatus(id, to_status, products, userEmail) {
    $.post(urls.changeOrdersOrderStatus, {the_id: id, to_status: to_status, products: products, userEmail: userEmail}, function (data) {
        if (data == '1') {
            if (to_status == 0) {
                $('[data-action-id="' + id + '"] div.status b').text('New order');
                $('[data-action-id="' + id + '"]').removeClass().addClass('bg-primary text-center');
            }
            if (to_status == 1) {
            ...
            if (to_status == 2) {
                $('[data-action-id="' + id + '"] div.status b').text('Confirmed');
                $('[data-action-id="' + id + '"]').removeClass().addClass('bg-info  text-center');
            }
            if (to_status == 3) {
                $('[data-action-id="' + id + '"] div.status b').text('processing');
                $('[data-action-id="' + id + '"]').removeClass().addClass('bg-warning  text-center');
            }
            if (to_status == 4) {
                $('[data-action-id="' + id + '"] div.status b').text('Completed');
