...
var elFinder = function(elm, opts, bootCallback) {
	...
	*/
	 ...
	 * @return String
	 */
	this.decorderawString = function(str) {
	...
	bootUp = function() {
		...
		self.trigger('init')
			...
			.done(function(data) {
				...
				// regist rawStringDecorder
				if (self.options.rawStringDecorder) {
					self.registRawStringDecorder(self.options.rawStringDecorder);
				}
...
elFinder.prototype = {
	...
	/**
	 * Regist this.decorderawString function
	 * 
	 * @return void
	 */
	registRawStringDecorder : function(rawStringDecorder) {
		if ($.isFunction(rawStringDecorder)) {
			this.decorderawString = this.options.rawStringDecorder = rawStringDecorder;
		}
	},
...
elFinder.prototype._options = {
	 ...
	 * It is used instead of fm.decorderawString(str)
	 * 
	 * @type Null|Function
	 */
	rawStringDecorder : typeof Encoding === 'object' && $.isFunction(Encoding.convert)? function(str) {
...
if (typeof elFinder === 'function' && elFinder.prototype.i18) {
	elFinder.prototype.i18.en = {
		...
		messages   : {
			...
			'errFolderUpload'      : 'Try the mordern browser, If you\'d like to upload the folder.', // from v2.1 added 26.6.2015
	...
	function(ql) {
			...
			amrToWavUrl = function(hash) {
				var dfd = $.Deferred(),
					...
					_AMR;
				if (window.TextEncorder && window.URL && URL.createObjectURL && typeof AMR === 'undefined') {
					// previous window.AMR
					_AMR = window.AMR;
					delete window.AMR;
					fm.loadScript(
						...
						makeList = function(filenames) {
							...
							if (filenames && filenames.length) {
								filenames = $.map(filenames, function(str) {
									return fm.decorderawString(str);
								});
								filenames.sort();
								loading.remove();
